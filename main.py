...
#from dataset.dataset_wifi impor WiFiDataset
from dataset import CSI_Dataset
from dataset import CSI_Dataset_val
...
parser.add_argument('--dataset', default='wifi', choices=['inat', 'imagenet','cifar10','wifi'])
parser.add_argument('--data', default='./data/wifi', metavar='DIR')
parser.add_argument('--arch', default='resnet32', choices=['resnet50', 'resnext50','resnet32'])
...
parser.add_argument('-b', '--batch-size', default=256, type=int, #128
                    metavar='N',
...
parser.add_argument('--beta', default=0.6, type=float, help='supervised contrastive loss weight')
...
parser.add_argument('--feat_dim', default=64, type=int, help='feature dimension of mlp head')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == 'resnet50':
        ...
        model = resnext.BCLModel(name='resnet32', num_classes=num_classes, feat_dim=args.feat_dim,
                                 use_norm=args.use_norm)
    else:
    ...
    # 这里要修改
    train_dataset = CSI_Dataset(root_dir='./data/wifi/maml_direction', transform=transform_train)
    print('train_dataset:', train_dataset)
    # train_dataset = ImbalanceCIFAR10(root='./data/cifar', train=True,
    #                              download=True, transform=transform_train)
    #
    val_dataset = CSI_Dataset_val(root_dir='./data/wifi/direction_dataset_20190719', transform=val_transform)
    ...
    criterion_ce = LogitAdjust(cls_num_list).cpu()
    criterion_scl = BalSCL(cls_num_list, args.temp).cpu()
...
def validate(train_loader, val_loader, model, criterion_ce, epoch, args, tf_writer=None, flag='val'):
    ...
    # total_logits = torch.empty((0, args.cls_num)).cpu()
